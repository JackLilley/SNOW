(function () {
  ...
  function renderNav(){
    var n=document.getElementById('ucNav');if(!n)return;n.innerHTML='';
  ...
  function showDlg(){
    ...
    function rebuildDlg(){
      ...
      var schedSection=el('div',{className:'uc-dlg-sched',style:showSched?'':'display:none'},[
        el('label',null,'Schedule date and time:'),
  ...
  function loadHist(){
    ...
    api('/api/now/table/sys_progress_worker?sysparm_display_value=all&sysparm_fields=sys_id,name,state,percent_complete,message,output_summary,sys_created_on&sysparm_query='+encodeURIComponent(q)+'&sysparm_limit=50')
      .then(function(d){
        ...
        if(!es.length){h.innerHTML='';h.appendChild(el('div',{className:'uc-empty'},'No installation history found.'));return;}
        h.innerHTML='';
      ...
      }).catch(function(err){console.error('[UC] History load error:',err);var h=document.getElementById('ucHist');if(h){h.innerHTML='';h.appendChild(el('div',{className:'uc-empty'},'Failed to load history.'));}});
  }
  ...
  function loadScheduledView(){
    ...
    api('/api/now/table/sys_progress_worker?sysparm_display_value=all&sysparm_fields=sys_id,name,state,message,output_summary,sys_created_on&sysparm_query='+encodeURIComponent(q)+'&sysparm_limit=50')
      .then(function(d){
        ...
        if(!items.length){h.innerHTML='';h.appendChild(el('div',{className:'uc-empty'},'No scheduled updates. Select updates and choose "Schedule for Later" to create one.'));return;}
        h.innerHTML='';
      ...
      }).catch(function(err){console.error('[UC] Scheduled load error:',err);var h=document.getElementById('ucSched');if(h){h.innerHTML='';h.appendChild(el('div',{className:'uc-empty'},'Failed to load scheduled updates.'));}});
  }
  ...
  function showReleaseNotes(summaryJson){
    ...
    var thead=el('thead',null,el('tr',null,[el('th'),el('th',null,'Application'),el('th',null,'From'),el('th'),el('th',null,'To'),el('th',null,'Status'),el('th',null,'Notes')]));
    var tbody=el('tbody',null,tblRows);
    var tbl=el('table',{className:'uc-rn-tbl'},[thead,tbody]);
